  ...
  for (var outputFile in this._scriptOutputFiles) {
    ...
    vendorFiles.push(
      concat(combined, {
        ...
        outputFile: 'bundles/' + this.name + outputFile,
        separator: EOL + ';',
    ...
    options.init = function() {
      ...
      this.included = function() {
        if (this.lazyLoading === true) {
          ...
          this.import('bundles/' + this.name + '/assets/engine-vendor.js');
          this.import('bundles/' + this.name + '/assets/engine-vendor.css');
        }
      ...
      this.treeForPublic = function() {
        ...
        if (engineStylesTree) {
          primaryStyleTree = concat(engineStylesTree, {
            allowNone: true,
            inputFiles: ['**/*.css'],
            outputFile: 'bundles/' + this.name + '/assets/engine.css'
          });
        }
        ...
        var childAddonsPublicTreesRelocated = new Funnel(childAddonsPublicTreesMerged, {
          exclude: ['bundles', 'bundles/**/*.*'],
          destDir: 'bundles/' + this.name
        });
        ...
        var concatEngineTree = concat(engineAppTreeWithoutRoutes, {
          allowNone: true,
          inputFiles: ['**/*.js'],
          outputFile: 'bundles/' + this.name + '/assets/engine.js'
        });
        ...
        var concatVendorJSTree = concat(vendorJSTree, {
          allowNone: true,
          inputFiles: ['**/*.js'],
          outputFile: 'bundles/' + this.name + '/assets/engine-vendor.js'
        });
        ...
        var concatVendorCSSTree = concat(vendorCSSTree, {
          allowNone: true,
          inputFiles: ['**/*.css'],
          outputFile: 'bundles/' + this.name + '/assets/engine-vendor.css'
        });
